...
**When to use**: After substantial sessions, after reorganization, or periodically to maintain vault health. For lighter end-of-session checks, use the `wrap-up.md` workflow instead.

## Usage

```
Ask Codex to follow `.codex/commands/vault-audit.md`
```

## Playbooks

This workflow can use two deeper playbooks for analysis:
...
Use `vault-librarian` first for the structural audit, then `cross-linker` for link quality if the
user wants the extra depth.
...
- Nothing unexpected at vault root (allowed: `Home.md`, `AGENTS.md`, `vault-manifest.json`, `CHANGELOG.md`, `CONTRIBUTING.md`, `README.md`, `LICENSE`, `.gitignore`, `.codex/` — no user notes)
...
- `brain/Skills.md` — are all playbooks registered? Workflows still valid?
...
### 11. Check Codex Config

- `.codex/commands/` — do the playbooks reference the correct folder structure?
- `.codex/playbooks/` — do the deeper workflows still match the vault layout?
- `.codex/scripts/` — do helper scripts still point at the right paths and conventions?
- `AGENTS.md` — any stale instructions that contradict current vault state?
...
- Use parallel or delegated work only when the user explicitly asks for it
