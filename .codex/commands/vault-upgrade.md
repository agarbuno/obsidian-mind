...
```
Follow `.codex/commands/vault-upgrade.md` with a source path
Follow `.codex/commands/vault-upgrade.md` with a source path and `--dry-run`
```
...
- **`vault-migrator`** — classifies files (for arbitrary vaults) and executes the approved migration plan when deeper migration help is needed
...
4. Check for `vault-manifest.json` in the source:
   ...
   - **Not found**: run version fingerprint detection from the target's `vault-manifest.json`:
     ...
     - Check for `brain/` + `bases/` without `.claude/agents/` and without `.codex/playbooks/` → v2
     - Check for `.claude/agents/` or `.codex/playbooks/` → later structured template (refine by checking the manifest when present)
...
| `.claude/commands/` or `.codex/commands/` not in template | **MERGE** | `.codex/commands/` (user addition) |
| `.claude/agents/` or `.codex/playbooks/` not in template | **MERGE** | `.codex/playbooks/` (user addition) |
| `.claude/scripts/` or `.codex/scripts/` not in template | **MERGE** | `.codex/scripts/` (user addition) |
...
⚠ Conflicts requiring your input:
  - source has customized AGENTS.md or CLAUDE.md (will NOT copy — differences logged)
  - source has a custom command or playbook file (will MERGE as user addition)
```
...
Migration complete.
  ...
  ⚠ 3 orphan notes (no inbound links) — run the vault-audit workflow to fix
...
Offer next steps:
- "Run the `vault-audit.md` workflow for a full structural check"
- "Review `thinking/migrate-review/` for uncategorized files"
- "Check `brain/Migration Log.md` for the full receipt and the instruction-file diff"
...
- **Instruction files are special.** Never copy `AGENTS.md` or `CLAUDE.md` from the source. The target template's instruction file is always authoritative. If the source has customizations, diff them and log the differences so the user can re-apply manually.
