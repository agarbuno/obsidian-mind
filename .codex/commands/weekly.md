...
- **`brag-spotter`** — run with weekly scope to find uncaptured wins and competency gaps when deeper analysis is helpful
...
Run the `brag-spotter` playbook with its standard (quarterly) scope, then filter its findings down to wins that clearly occurred in the past 7 days.
