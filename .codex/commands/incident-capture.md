...
```
Follow `.codex/commands/incident-capture.md` with one or more Slack URLs or transcript sources
```
...
This workflow uses two deeper playbooks for heavy lifting:
...
Use them in parallel only when the user explicitly asks for delegated or parallel work.
...
- "Should I run the `vault-audit.md` workflow to verify everything links properly?"
