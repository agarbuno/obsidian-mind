---
name: vault-migrator
description: "Classify, transform, and migrate vault content from a source vault into this obsidian-mind instance. Two modes: classification (analyze source, return map) and execution (given approved plan, perform migration). Invoked by the vault-upgrade workflow."
...
**Custom workflow files** (`.claude/` or `.codex/` commands, playbooks, or scripts not in template):
- Copy them into the matching target `.codex/` directory
...
- AGENTS.md diff summary (if source had a customized AGENTS.md, list the differences so the user knows what conventions to re-apply)
...
- Mostly structural parity. Check for missing `.codex/playbooks/` and `.codex/scripts/` (v2 didn't have them — but don't copy, the template already has latest)
